...
const PostItem = ({profile:{profiles}, deletePost, addLike, removeLike, auth, post: {_id, text, name, avatar, user, likes, comments, date}, showActions}) => {
          ...
          <div>
            ...
            <div className="post-down">
              ...
              <div className="post-index">
                ...
                <button onClick={e => addLike(_id)} type="button" className="thumb-up discussion">
                  <div style={{width: '50px'}}>{likes.length > 0 && (<span className='comment-count'>{likes.length}</span>)}</div>
                  <ThumbUpIcon />{' '}
                </button>
                
                <Link to={`/posts/${_id}`} className="discussion">
                   {comments.length > 0 && (<div style={{width: '100%'}}><span className='comment-count'>{comments.length}</span></div>)}
                    <MessageIcon />
