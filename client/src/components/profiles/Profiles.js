...
const Profiles = ({ getProfilesLoad, profile:{profiles, loading}, postSize}) => {
    ...
    const [props, setPara] = useSpring(() => ({ xy: [0, 0], config: {mass: 1, tension: 280, friction: 120}}))
    ...
    <section className="container">
        ...
        <div className="buddy-box" onMouseMove={({ clientX: x, clientY: y }) => setPara({ xy: calc(x, y) })}>
          ...
          <div className="right-buddy">              
            <animated.div class="buddy1 buddy" style={{ transform: props.xy.interpolate(trans1) }} />
            <animated.div class="buddy2 buddy" style={{ transform: props.xy.interpolate(trans2) }} />
            <animated.div class="buddy3 buddy" style={{ transform: props.xy.interpolate(trans3) }} />
            <animated.div class="buddy4 buddy" style={{ transform: props.xy.interpolate(trans4) }} />
            ...
            <div className="filter-logan">Looking for people who are interested in...</div>
            ...
            {profiles.length > 0 ? (profiles.map(profile => ( <ProfileItem key={profile._id} profile={profile} />))) : <SpinnerProfiles />}
            </div>
            {postSize >= Limit && <div style={{ display:'flex',marginTop:'20px', justifyContent:'center'}}>
